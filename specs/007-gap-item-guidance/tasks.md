...
- [ ] T001 Confirmar branch `007-gap-item-guidance` e os reaproveitamentos: `gap_seed_item.objective`
...
- [ ] T002 [P] Revisar pontos de reuso: `gap_seed_service.load_seed`/`adopt_seed`
...
- [ ] T003 Adicionar colunas de orientação a `GapSeedItem` em `wtnapp/models/gap_seed_model.py`:
...
- [ ] T004 [P] Criar modelo `GapLegendEntry` em `wtnapp/models/gap_legend_model.py` (platform, **sem
  `tenant_id`**): `kind` (status|priority), `code`, `label`, `definition`, `order`; único por (kind,code).
- [ ] T005 [P] Criar modelo `GapGuidanceEvent` (append-only, platform, **sem `tenant_id`**) em
...
- [ ] T006 [P] Criar DTOs em `wtnapp/schemas/gap_guidance_schema.py`: `ItemGuidance`, `LegendEntry`,
  `Legend`, `GuidanceResponse`, `ItemGuidanceUpdate`, `LegendEntryUpdate`, `GuidanceEvent`
  (conforme [data-model.md](data-model.md)).
- [ ] T007 Criar migration `wtnapp/alembic/versions/<rev>_gap_guidance.py` com
...
- [ ] T009 [P] [US1] Teste de leitura em `wtnapp/test/test_gap_guidance.py`: `GET /gap/guidance`
...
- [ ] T010 [P] [US1] Teste de RBAC de leitura em `wtnapp/test/test_gap_guidance.py`: usuário sem
  `view_gap` ⇒ 403.

### Implementation for User Story 1
- [ ] T011 [US1] Implementar `get_guidance(db)` em `wtnapp/services/gap_guidance_service.py` (itens do
  seed corrente + legenda, no formato `GuidanceResponse`).
- [ ] T012 [US1] Criar `wtnapp/routers/gap_guidance.py` com `GET /gap/guidance`
  (`require_permission("view_gap")`) e **registrar em `wtnapp/main.py`**.
- [ ] T013 [US1] **Conteúdo** — autorar `referencia`/`como_avaliar`/`evidencias_esperadas`/`nota`
  (PT-BR **original**) das **7 cláusulas (4–10)** em `wtnapp/data/iso27001_seed.py`.
- [ ] T014 [US1] **Conteúdo** — Anexo A **A.5 Organizacional (37 controles)** em
  `wtnapp/data/iso27001_seed.py`.
- [ ] T015 [US1] **Conteúdo** — Anexo A **A.6 Pessoas (8)** + **A.7 Físico (14)** em
  `wtnapp/data/iso27001_seed.py`.
- [ ] T016 [US1] **Conteúdo** — Anexo A **A.8 Tecnológico (34)** em `wtnapp/data/iso27001_seed.py`.
- [ ] T017 [P] [US1] Frontend: em `wtnadmin/src/app/pages/gap-analysis/gap-analysis.ts`, buscar
...
- [ ] T019 [P] [US2] `wtnapp/test/test_gap_guidance.py`: Super Admin edita campo de item ⇒ persistido;
  `gap_guidance_event` registra `old→new`; audit gerado; nova leitura reflete; **propagação** (edição
  vista por leitura de outra org).
- [ ] T020 [P] [US2] **Teste de RBAC/isolamento** em `wtnapp/test/test_gap_guidance_rbac.py`:
...
- [ ] T021 [P] [US2] `wtnapp/test/test_gap_guidance.py`: rodar `load_seed` após edição **não**
  sobrescreve; UPDATE/DELETE em `gap_guidance_event` falha (append-only).

### Implementation for User Story 2
- [ ] T022 [US2] Implementar `update_item_guidance(db, seed_item_id, patch, actor)` e
...
- [ ] T023 [US2] Adicionar em `wtnapp/routers/gap_guidance.py`: `PUT /gap/guidance/items/{seed_item_id}`,
  `PUT /gap/guidance/legend/{entry_id}` e `GET /gap/guidance/events` — todos `require_super_admin`.
- [ ] T024 [P] [US2] Frontend: criar `wtnadmin/src/app/pages/gap-guidance-admin/gap-guidance-admin.ts`
...
- [ ] T025 [P] [US2] Frontend: `wtnadmin/src/app/pages/gap-guidance-admin/gap-guidance-admin.spec.ts`
...
- [ ] T029 [P] Atualizar status da Feature 007 em `CLAUDE.md` (planejada → implementada, com contagem
  de testes) + seção do módulo.
- [ ] T030 **Audit review**: confirmar que edições (item/legenda) e tentativas negadas são auditadas,
  sem PII/segredos nos eventos/audit; leitura não loga.
- [ ] T031 **Revisão de IP do conteúdo**: confirmar que os 100 itens têm orientação **original** em
  PT-BR — nenhuma reprodução de texto normativo da ISO/IEC 27001/27002 (FR-011).
- [ ] T032 Rodar validação do [quickstart.md](quickstart.md): `pytest wtnapp/test/test_gap_guidance.py
